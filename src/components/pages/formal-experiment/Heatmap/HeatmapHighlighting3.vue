<template>
  <div class="heatmap-highlighting-task">
    <!-- 页面标题 -->
    <h2 class="page-title">Heatmap + Highlighting (3/3)</h2>
    ...
    <div class="content-container">
      ...
      <div class="task-section">
        ...
        <h2 class="task-title">
          Task 3: Compare <em>spring</em> (<em>March-May</em>) and
          <em>fall</em> (<em>September-November</em>). In which season are there more days with a
          temperature difference of more than <em>10°C</em>?
        </h2>
        ...
        <div class="button-control">
          <button
            class="confirm-btn"
            @click="handleConfirm(null, selectedAnswer, '/heatmap-staging-1')"
...
const { handlePlayAnimation, handleConfirm } = useTaskTimer(
  'heatmap-highlighting-3',
  'Heatmap + Highlighting (3/3)',
)
...
const options = [
  {
    letter: 'A',
    value: 'A',
    text: 'More days in <em>spring</em>',
  },
  {
    letter: 'B',
    value: 'B',
    text: 'More days in <em>autumn</em>',
  },
  {
    letter: 'C',
    value: 'C',
    text: 'Almost the same number of days in both seasons',
  },
  {
    letter: 'D',
    value: 'D',
    text: 'No days in either season have a temperature difference of more than <em>10°C</em>',
  },
]

onMounted(() => {
  initHeatmapHighlighting()
})

const initHeatmapHighlighting = async () => {
  try {
    // 数据存储
    ...
    const applyHighlighting = () => {
      const TEMP_DIFFERENCE_THRESHOLD = 10
      for (let m = 1; m <= 12; m++) {
        for (let d = 1; d <= DAYS_IN_MONTH[m - 1]; d++) {
          const key = `${m}-${d}`
          const moscowPoint = dataMoscow.get(key)
          const copenhagenPoint = dataCopenhagen.get(key)
          if (moscowPoint && copenhagenPoint) {
            const diff = Math.abs(moscowPoint.avgTemp - copenhagenPoint.avgTemp)
            if (diff > TEMP_DIFFERENCE_THRESHOLD) {
              document.getElementById(`cell-moscow-${m}-${d}`).classList.add('highlighted')
              document.getElementById(`cell-copenhagen-${m}-${d}`).classList.add('highlighted')
            }
          }
        }
      }
    ...
    renderChartGrid('Moscow', dataMoscow)
    renderChartGrid('Copenhagen', dataCopenhagen)
    console.log('图表网格渲染完成')
    ...
    window.heatmapData = { dataMoscow, dataCopenhagen, applyHighlighting, DAYS_IN_MONTH }
  } catch (err) {
    console.error(err)
...
const playAnimation = () => {
  ...
  const { applyHighlighting } = window.heatmapData
