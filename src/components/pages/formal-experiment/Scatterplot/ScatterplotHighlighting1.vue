...
const initScatterplot = () => {
  ...
  // Create the plot with highlighting animation
  function createPlot(dailyData) {
    ...
    // Create initial plot with all gray points
...
const playAnimation = () => {
  ...
  const { monthColors } = chartData

  // Reset all traces to gray first using restyle
  for (let month = 1; month <= 12; month++) {
    const traceIndex = month - 1
  ...
  // After 1 second, highlight summer months (6, 7, 8)
  setTimeout(() => {
    const summerMonths = [6, 7, 8]

    // Update the marker colors for summer months
    summerMonths.forEach((month) => {
      const traceIndex = month - 1 // Trace index is month - 1 (0-indexed)
      window.Plotly.restyle(
        'scatterplot-chart',
        {
          'marker.color': monthColors[month - 1],
        },
        [traceIndex],
      )
    })
