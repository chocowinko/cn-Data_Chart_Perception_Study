...
const playAnimation = () => {
  ...
  // Calculate 90th percentile (top 10%) threshold first
  const sortedPM25 = [...pm25Values].sort((a, b) => a - b)
  const percentileIndex = Math.floor(sortedPM25.length * 0.9)
  const pm25Threshold = sortedPM25[percentileIndex]

  // Find indices of high PM2.5 concentration points
  const highPM25Indices = []
  pm25Values.forEach((pm25, i) => {
    if (pm25 > pm25Threshold) {
      highPM25Indices.push(i)
    }
  })

  // Create double traces: colored invisible traces for legend, gray/colored visible traces for data
  const scatterTraces = []

  // First, add invisible colored traces just for the legend
  for (let month = 1; month <= 12; month++) {
    scatterTraces.push({
      x: [null],
      y: [null],
      ...
      showlegend: true,
      hoverinfo: 'none',
      legendgroup: `month-${month}`,
    })
  }

  // Then, add visible traces for the actual data with mixed colors
  for (let month = 1; month <= 12; month++) {
    const monthIndices = monthValues.map((m, i) => (m === month ? i : -1)).filter((i) => i !== -1)
    if (monthIndices.length > 0) {
      // Create color array: colored for high PM2.5, gray for others
      const colors = monthIndices.map((globalIndex) => {
        return highPM25Indices.includes(globalIndex) ? monthColors[month - 1] : '#F1F1F1'
      })
