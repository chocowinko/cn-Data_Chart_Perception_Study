<template>
  <div class="line-graph-task">
    <!-- 页面标题 -->
    <h2 class="page-title">Line Graph + Highlighting (2/3)</h2>
    ...
    <div class="content-container">
      ...
      <div class="task-section">
        <h3 class="event-description">
          Highlighting Event: The lowest sales point (trough) for each product line throughout the
          year.
        </h3>

        <h2 class="task-title">
          Task 2: Of the highlighted annual sales troughs, which category reached the absolute
          lowest point?
        </h2>
        ...
        <div class="button-control">
          <button
            class="confirm-btn"
            @click="handleConfirm(null, selectedAnswer, '/line-graph-highlighting-3')"
...
const { handlePlayAnimation, handleConfirm } = useTaskTimer(
  'line-graph-highlighting-2',
  'Line Graph + Highlighting (2/3)',
)
...
const options = [
  {
    letter: 'A',
    value: 'A',
    text: '<em>Sports & Recreation</em>',
  },
  {
    letter: 'B',
    value: 'B',
    text: '<em>Furniture</em>',
  },
  {
    letter: 'C',
    value: 'C',
    text: '<em>Construction & Decoration Materials</em>',
  },
  {
    letter: 'D',
    value: 'D',
    text: '<em>Tobacco & Alcohol</em>',
  },
]
...
const playAnimation = () => {
  ...
  // 1秒后高亮所有最低点
  setTimeout(() => {
    highlightAllLowestPoints(volatilityData, g, distinctColors, xScale, yScale, rawData, tooltip)
  }, 1000)
}
...
function highlightAllLowestPoints(
  volatilityData,
  svgGroup,
  distinctColors,
  xScale,
  ...
  volatilityData.forEach((item, index) => {
    ...
    console.log(
      `${item.category} 最低点: ${minValue}亿元，月份: ${minIndices.map((i) => rawData.months[i]).join(', ')}`,
    )

    const originalColor = distinctColors[index % distinctColors.length]
    const saturatedColor = increaseSaturation(originalColor, 90)

    // 为每个最低点添加高亮圆圈
    minIndices.forEach((minIndex) => {
      svgGroup
        .append('circle')
        .attr('class', 'highlight-dot')
        .attr('cx', xScale(rawData.months[minIndex]))
        .attr('cy', yScale(minValue))
        .attr('r', 0)
        .style('fill', saturatedColor)
        .style('stroke', saturatedColor)
        .style('stroke-width', 2)
        .style('opacity', 0)
        .transition()
        .duration(500)
